"""
lv_levels.py — Alle level-definities voor NP-Hard Pac-Man.

Elke entry in LEVELS is een dict die één level volledig beschrijft.
De gamelogica (lv_logic.py) en de visuele renderer (level1.py) lezen hieruit.

Level-structuur
───────────────
Serie 1 (1A-1F)  — index 0-5:   Basisregels: muren, geest, teleporters, powerup
Serie 2 (2A-2F)  — index 6-11:  Meta-regels: teal/paarse/oranje muren,
                                 hinderlaag-geest, gedeelde teleporters,
                                 vertraagde powerup, meta-dozen
Serie 3 (3A-3F)  — index 12-17: Meta-meta-regels: local rule zone met
                                 perimeterpatrouille, one-shot teal muren,
                                 delayed PU pause, wallbreaking meta-doos
Serie 4 (4A)     — index 18:    Wager mechanic, 8×8 grid, ghost A4↔G4
Serie 4 (4B)     — index 19:    Wager mechanic, 12×12 grid, ghost G6↔L6
Serie 4 (4C)     — index 20:    Estimate mechanic, 8×8 grid
Serie 4 (4D)     — index 21:    Estimate mechanic, 12×12 grid

Muur-notatie
────────────
Muren worden opgeslagen als frozenset van twee cel-coördinaten:
  frozenset([(col1, row1), (col2, row2)])
De helpers _e() en _n() maken deze frozensets leesbaar in de definitie:
  _e(col, row)  →  verticale muur: linkerrand van cel (col, row)
  _n(col, row)  →  horizontale muur: bovenrand van cel (col, row)

Kleurenpalettoewijzing (Paul Tol Muted, kleurenblindveilig)
────────────────────────────────────────────────────────────
  walls        → ROSE    (194,106,119)  permanente muren
  green_walls  → TEAL    ( 93,168,153)  one-shot muren in zone (code-naam "green" is historisch)
  purple_walls → PAARS   (159, 74,150)  permanente extra muren
  orange_gates → ZANDGEEL(220,205,125)  one-shot gates (open); donker zandgeel na gebruik

Elke level-dict heeft de volgende sleutels
──────────────────────────────────────────
  title             str      weergavenaam (bv. "LEVEL 1A")
  cols, rows        int      grid-afmetingen
  cell              int      celgrootte in pixels (alleen voor de visuele game)
  start             (c, r)   startpositie van de speler
  goals             list     lijst van {"pos": (c,r), "type": "banana"}
  max_moves         int      maximaal aantal zetten voor game_over
  walls             set      rose (permanente) muren als frozensets
  green_walls       set      teal (one-shot in zone) muren — visueel lichtblauw/teal
  purple_walls      set      paarse (permanente) muren
  orange_gates      list     one-shot gates met richting en bidirectioneel-vlag
  shared_orange_gates list   one-shot gates die ook de geest blokkeren
  gate              dict|None  optionele gate {"pos":(c,r), "open": bool}
  ghost             dict|None  patrouille-geest definitie
  ambush_ghost      dict|None  hinderlaag-geest definitie
  teleporters       list|None  [portaal_A, portaal_B] — alleen voor speler
  shared_teleporters list|None  idem, maar ook geesten mogen erdoor
  ghost_teleporters list|None  alleen voor geesten (speler mag er niet door)
  powerup           dict|None  {"pos": (c,r)} — directe powerup
  delayed_powerup   dict|None  {"pos": (c,r)} — vertraagde powerup
  require_ghost_eaten bool    True → alle geesten moeten gegeten worden om te winnen
  pushable_blocks   list     startposities van duwbare dozen [(c,r), ...]
  meta_blocks       list     startposities van meta-dozen (schuivend)
  local_rule_zone   dict|None  zone met speciale regels (alleen serie 3+)
  wager_level       bool     True → level heeft preview + wager-fase (serie 4)
"""

# ─── Muur-helpers ─────────────────────────────────────────────────────────────

def _e(col, row):
    """Verticale muur op de LINKERRAND van cel (col, row).

    Slaat de overgang tussen (col-1, row) en (col, row) op als frozenset.
    Gebruik: _e(3, 2) = muur tussen kolom B en C op rij 3 (0-geïndexeerd).
    """
    return frozenset([(col - 1, row), (col, row)])

def _n(col, row):
    """Horizontale muur op de BOVENRAND van cel (col, row).

    Slaat de overgang tussen (col, row-1) en (col, row) op als frozenset.
    Gebruik: _n(2, 4) = muur tussen rij 4 en 5 in kolom C (0-geïndexeerd).
    """
...
{
    # LEVEL 2A — teal/paarse/oranje muren, ambush-geest, gedeelde teleporter
    "title": "LEVEL 2A",
...
{
    # LEVEL 2B — teal muren, ambush-geest, oranje poort, powerup-meta
    "title": "LEVEL 2B",
    "cols": 8, "rows": 8, "cell": 78,
    "start": (0, 0),
    "goals": [
...
{
    # LEVEL 2C — vertraagde powerup, ambush-geest, normale teleporter
    "title": "LEVEL 2C",
    "cols": 8, "rows": 8, "cell": 78,
    "start": (0, 7),   # A8
    "goals": [
...
{
    # LEVEL 2D — gedeelde teleporter (teleporter-meta), ambush-geest, vertraagde powerup
    "title": "LEVEL 2D",
    "cols": 8, "rows": 8, "cell": 78,
    ...
    "shared_teleporters":  [(2, 3), (5, 3)],   # D4 ↔ F4 (speler én geest)
...
},
{
    ...
    #            meta-block E2, normal block C8, bananen D8 + A8
    "title": "LEVEL 2E",
    "cols": 8, "rows": 8, "cell": 78,
    "start": (7, 0),   # H1
    "goals": [
    ...
    "max_moves": 58,
    "walls": {
        # ── Rode horizontale muren ────────────────────────────────────────────
        _n(6, 1),                                # G tussen rij 1-2
        _n(6, 2), _n(7, 2),  _n(5, 6),                      # G en H tussen rij 4-5
        _n(1, 6), _n(2, 6), _n(3, 6),_n(7, 6), # B,C,E,H tussen rij 6-7
                                                 # (D en G zijn one-shot gates)
        # ── Rode verticale muren ──────────────────────────────────────────────
        _e(5, 1), _e(5, 2),   # E-F op rij 2 en 3
        _e(6, 4), _e(7, 5),_e(6, 2),_e(6, 3),   # F-G op rij 5, G-H op rij 6
        _e(2, 6),  # B-C op rij 7 en 8
        _e(1, 7),              # A-B op rij 8
        _e(5, 6),_e(6, 5),    _e(1, 0),           # E-F op rij 7
    },
    "green_walls": {
        _e(5, 7),   # E-F op rij 8 (verticaal)
        _n(0, 6),   # A tussen rij 6-7 (horizontaal)
    },
    "purple_walls": set(),
    "orange_gates": [
    ...
    "require_ghost_eaten": True,
    "pushable_blocks":     [(2, 7)],           # C8 — normale doos
...
},
{
    ...
    "green_walls": {
        # ── Groene horizontale muren ────────────────────────────────────────────
        _n(0, 1), _n(1, 4), _n(1, 7),_n(2, 7), _n(7, 2), 
        # ── Groene verticale muren ──────────────────────────────────────────────
         _e(6, 5),   
    },
    "purple_walls": set(),
    "orange_gates": [
    ...
    "require_ghost_eaten": True,
    "pushable_blocks":     [(5, 2)],           # C8 — normale doos
    "meta_blocks":         [(2, 3)],           # E2 — sliding meta-block
},

# ── LEVEL 3 series: meta-meta-regels (local rule zones) ──────────────────────
{
    # LEVEL 3A — 12×12 grid, local rule zone B5-E8
    # Zone-effect: ambush-geest die de zone betreedt schakelt over naar
    #              kloksgewijze randpatrouille van de zone-rand.
    #
    # Grid 12×12  (A-L = 0-11,  rijen 1-12 = 0-11)
    # Speler start:      A1  (0,0)
    # Bananen:           L1 (11,0)  K5 (10,4)  A12 (0,11)
    # Patrol ghost:      G1↔K1  (kolommen 6-10, rij 0, horizontaal)
    # Ambush ghost:      I5 (8,4), radius 2
    # Local rule zone:   B5-E8  → rect(1,4,4,7)
    # Shared teleporters: K3 (10,2) ↔ K10 (10,9)
    # Normale teleporters: A10 (0,9) ↔ L12 (11,11)
    # Meta-movable block: C10 (2,9)
    # Normale doos:       L8 (11,7)
    # Powerup (geel):     H11 (7,10)
    # Delayed powerup:    C3 (2,2)
    # One-shot gate:      K↔L5 (bidirectioneel)
    "title": "LEVEL 3A",
    ...
    "max_moves": 75,
    "walls": {
        # ── Rode horizontaal (= verticale lijnen, tussen kolommen) ────────────
        _e(3,  0),   # C-D  rij 1
        _e(6,  0),   # F-G  rij 1
        _e(7,  2),   # G-H  rij 3
        _e(8,  3),   # H-I  rij 4
        ...
        _e(8,  10),  # H-I  rij 11
        _e(4,  11),  # D-E  rij 12
        # ── Rode verticaal (= horizontale lijnen, tussen rijen) ───────────────
        _n(8,  1),   # I   rij 1-2
        _n(11, 1),   # L   rij 1-2
        _n(2,  2),   # C   rij 2-3
        _n(2,  3),   # C   rij 3-4
    ...
    "green_walls": {
        # ── Groene horizontaal (= verticale lijnen, tussen kolommen) ──────────
        _e(1,  1),   # A-B  rij 2
        _e(1,  2),   # A-B  rij 3
        _e(6,  5),   # F-G  rij 6
        _e(6,  6),   # F-G  rij 7
        ...
        _e(4,  10),  # D-E  rij 11
        _e(11, 10),  # K-L  rij 11
        # ── Groene verticaal (= horizontale lijnen, tussen rijen) ─────────────
        _n(4,  1),
        _n(4,  3),   # E   rij 1-2
        _n(0,  2),   # A   rij 2-3
        _n(1,  2),   # B   rij 2-3
        ...
        _n(5,  9),   # F   rij 9-10
        _n(4,  10),  # E   rij 10-11
        _n(11, 10),  # L   rij 10-11
        _n(10, 11),  # K   rij 11-12
        _n(11, 11),  # L   rij 11-12
    },
    "purple_walls": set(),
    "orange_gates": [
        # K↔L rij 5: bidirectioneel eenmalig
...
},
{
    ...
    "walls": {
        ...
        _e(3,  10),_e(3,  11),_e(5,  10),_e(5,  11),_e(7,  10),_e(7,  11),_e(9,  11),  # C-D  rij 1
...
# Teleporters A2↔D3. Powerup G5. Pushable box D6.
{
    ...
    "optimal_moves":       18,
    "walls":               {
        # ── Horizontale muren (tussen rij r en r+1) ──────────────────────────
        ...
        # ── Verticale muren (tussen col c en c+1) ────────────────────────────
...
# Teleporters A2↔A8. Powerup G5. Movable block B1.
{
    ...
    "ghost":               {
        ...
        "waypoints": [
            ...
            (5, 5),   # F6
            (6, 5),   # G6
            (6, 4),   # G5  ← keerpunt (muur G4,5 blokkeert G4→G3)
            (6, 5),   # G6
            (5, 5),   # F6
            (4, 5),   # E6
            (4, 4),   # E5
