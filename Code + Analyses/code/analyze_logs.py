"""
analyze_logs.py — Cognitive flexibility analysis voor NP-Hard Pac-Man runs.

Leest alle JSON-logbestanden uit logs/ en berekent per model een
cognitief flexibiliteitsprofiel op basis van drie kernmaten:

  blocked_rate       — fractie zetten die geblokkeerd waren (lager = efficiënter)
  perseveration_rate — fractie geblokkeerde zetten die al eerder geblokkeerd waren
                       op dezelfde positie/richting (lager = minder perseveratie)
  switch_cost        — verschil in blocked_rate voor vs na eerste zone-entry
                       (negatief = beter na regelwisseling; positief = slechter)

Gebruik
───────
  python analyze_logs.py                      # alle logs in logs/
  python analyze_logs.py --dir my_logs/       # aangepaste map
  python analyze_logs.py --level "Level 1A"   # filter op level-naam
  python analyze_logs.py --model claude        # filter op model-prefix
  python analyze_logs.py --csv output.csv     # exporteer naar CSV
"""
