"""
level1.py — Visuele pygame-game voor NP-Hard Pac-Man.

Verantwoordelijk voor alles wat de speler ziet en invoer verwerkt.
De spellogica (wat er inhoudelijk gebeurt) staat in lv_logic.py;
dit bestand roept die functies aan en tekent het resultaat.

Scherm-modi
───────────
  "intro"             →  openingsscherm (druk een toets om te starten)
  "preview"           →  wager-levels: toon het level N seconden voor het bieden
  "wager"             →  slider om extra zetten te kopen/verkopen voor coins
  "playing"           →  spelscherm met grid, HUD en invoer
  "series_transition" →  tussenscherm na het afronden van een serie (1→2, 2→3, 3→4)
  "end_screen"        →  eindscherm na het afronden van alle levels

Tekenorde per frame (spelscherm)
─────────────────────────────────
  1. Achtergrond + gridcellen + muren/labels     (draw_grid)
  2. Level-titel                                 (draw_level_title)
  3. Local rule zone overlay                     (draw_local_rule_zone)
  4. Gedeelde teleporters                        (draw_shared_teleporter)
  5. Oranje gates                                (draw_orange_gate_wall)
  6. Gate                                        (draw_gate)
  7. Powerups                                    (draw_lightning / draw_delayed_powerup)
  8. Doelen / bananen                            (draw_banana / draw_food)
  9. Dozen                                       (draw_box / draw_meta_box)
  10. Geesten + patrouillespoor                  (draw_ghost / draw_ambush_ghost)
  11. Pac-Man                                    (draw_pacman)
  12. HUD (zettenbalk, scores)                   (draw_hud)
  13. Game-over / level-complete overlay

Animaties
─────────
  De variabele `t` loopt op in seconden (60 fps).  Alle animaties zijn
  sinusoïden over t: bobbing, mond-animatie, kleur-pulsering, etc.
  Dit zorgt voor vloeiende animaties zonder toestandsmachines.

Invoer
──────
  Pijltjestoetsen of WASD → bewegen
  R                        → huidig level herstarten
  SPACE                    → bevestigen (wager / volgende level)
  ESC                      → afsluiten
"""
...
def _save_run_log(lvl, attempt_num, moves_log, result, moves_used, implicit=False,
    ...
    """Sla één poging op als JSON in de sessie-map met samenvatting en metrics."""
    ...
    # Tijd vanaf level-start tot laatste zet (inclusief eerste zet zonder vorige)
    wall_time = None
...
def _save_session_summary(session_files, player_name="", implicit=False, session_dir=None):
    """Genereer één samenvattingsdocument van alle gespeelde levels in deze sessie."""
    target_dir = session_dir or RUNS_DIR
    os.makedirs(target_dir, exist_ok=True)
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
...
def _load_leaderboard(implicit=False):
    """Laad leaderboard-lijst uit JSON; geeft [] terug als bestand niet bestaat."""
...
def cell_rect(col, row, lvl):
    """Geeft het pygame.Rect van cel (col, row) in schermcoördinaten.
    PADDING verschuift het grid zodat er ruimte is voor de kolom-/rij-labels.
    """
...
def draw_wall_edge(surf, edge, lvl):
    """Tekent een rode permanente muur tussen twee cellen.
    Bepaalt automatisch of het een verticale of horizontale muur is op basis
    van de rij-coördinaten van de twee cellen in de frozenset.
    De kleine rechthoekjes aan de uiteinden zijn decoratieve eindkapjes.
    """
...
def draw_colored_wall(surf, edge, color, lvl, width=4):
    """Tekent een gekleurde muur (blauw/groen of paars) met dezelfde geometrie
    als draw_wall_edge maar met instelbare kleur en lijndikte.
    Gebruikt voor groene (one-shot) en paarse (permanente) muren.
    """
    (c1, r1), (c2, r2) = tuple(edge)
    cell = lvl["cell"]
    if r1 == r2:
...
def draw_orange_gate_wall(surf, og, used, lvl):
    """Tekent een oranje one-shot gate.
    Open gates (nog niet gebruikt) krijgen een pijl die de doorlaatrichting aangeeft;
    bidirectionele gates krijgen pijlen aan beide kanten.
    Gebruikte/gesloten gates worden donker geteal getekend zonder pijl.
    """
    edge     = og["edge"]
...
def draw_gate(surf, col, row, lvl, open_, t):
    """Tekent de reguliere gate (deur) in een cel.
    Open gate: twee poortpalen + boog + groen oplichtende doorgang.
    Gesloten gate: rood gevuld vakje met tralies en een middellijn.
    De parameter t geeft de animatietijd zodat de gloei kan pulseren.
    """
...
def draw_teleporter(surf, col, row, lvl, t, color_inner, color_outer):
    """Tekent een roterend teleportaal met concentrische ringen.
    Elke ring is een semi-transparante cirkel op een roterende positie.
    color_inner en color_outer bepalen het kleurverloop van binnen naar buiten.
    De twee eindpunten van een teleporterpaar krijgen elk een andere fase (t vs t+π)
    zodat ze visueel gespiegeld draaien.
    """
    rect  = cell_rect(col, row, lvl)
...
def draw_shared_teleporter(surf, col, row, lvl, t, is_first=True):
    """Gedeeld teleportaal (speler én geest) — inverse kleuren."""
    if is_first:
...
def draw_ghost_teleporter(surf, col, row, lvl, t, is_first=True):
    """Ghost-only teleportaal — geest-kleur (speler kan dit portaal niet gebruiken)."""
    # Grijs-blauwachtige tint om te onderscheiden van gedeelde TPs
    ...
    # Klein geest-icoontje in het midden om het type aan te duiden
    rect = cell_rect(col, row, lvl)
    cx, cy = rect.centerx, rect.centery
...
def draw_delayed_powerup(surf, col, row, lvl, t, countdown=0):
    """Vertraagde power-up — inverse kleur van normale bliksem."""
    rect = cell_rect(col, row, lvl)
    cx, cy = rect.centerx, rect.centery + int(3 * math.sin(t * 4))
    s = int(lvl["cell"] * 0.28)
    pts = [(cx + s*0.1,  cy - s),
...
def draw_meta_box(surf, col, row, lvl):
    """Meta-box (blauw) — schuift door tot een obstakel."""
    rect  = cell_rect(col, row, lvl)
    m     = 8
    inner = pygame.Rect(rect.x + m, rect.y + m, rect.w - 2*m, rect.h - 2*m)
    pygame.draw.rect(surf, (10, 10, 20),
...
def draw_grid(surf, lvl, font_small, t, used_green_walls=None, broken_walls=None):
    """Tekent de basislaag van het speelveld: cellen, muren en coördinaat-labels.

    Tekenorde binnen draw_grid:
      1. Celachtergronden (donkerblauw) + celrand (C_GRID)
      2. Gewone teleporters (voor de muren, zodat muren er bovenop liggen)
      3. Ghost-only teleporters
      4. Rode muren (lvl["walls"]) — gebrokenen overgeslagen
      5. Groene (one-shot) muren — gebruikt zijn donkerder
      6. Paarse muren
      7. Kolom-labels (A, B, C…) boven het grid
...
def draw_pacman(surf, col, row, lvl, t, direction, powered):
    """Tekent Pac-Man als een geanimeerde pizza-wig.
    De mondhoek oscilleert via sin(t*5) tussen bijna-dicht en 35°.
    `direction` bepaalt de basisrotatie van de wig (0° = rechts).
    `powered` = True → oranje kleur + gloei-effect rondom Pac-Man.
    """
    rect  = cell_rect(col, row, lvl)
    cx, cy = rect.centerx, rect.centery
    r      = int(lvl["cell"] * 0.28)
...
def draw_local_rule_zone(surf, lrz, lvl, t):
    """Tekent de local rule zone als een gekleurde semi-transparante overlay.

    De zone wordt getekend vóór de geesten en speler zodat alle elementen
    er bovenop verschijnen.  De rand pulseert licht om de zone te accentueren.
    """
    ...
    # Achtergrond-tint (Tol purple — onderscheidend van teal meta-muren)
...
def _draw_controls_panel(surf, font_big, font_small, x, y, w, h):
    """Controls-legenda voor de impliciete modus (links van het grid)."""
    controls = [
    ...
    for key, desc in controls:
        # schaal tekst zodat de toetslabel altijd binnen de breedte past
...
def draw_implicit_legend_panel(surf, font_big, font_small, lvl_title, x, y, w, fs_h,
    ...
    """Toont in impliciete modus de bekende elementen (met uitleg) + controls in één paneel."""
    ...
    # Klem scroll zodat je niet voorbij het einde scrolt
    ...
    # Clip-rect zodat content niet buiten het paneel tekent
    ...
    # ── Scrollbar (alleen als er meer content is dan zichtbaar) ──────────────
    if max_scroll > 0:
        sb_x      = x + w - SB_W - 2
...
def _implicit_legend_content_h(font_big, font_small, lvl_title, panel_w):
    """Bereken de totale inhoudshoogte van het impliciete legendapaneel voor lvl_title."""
    entries  = IMPLICIT_LEGEND.get(lvl_title, [])
...
def draw_hud(surf, lvl, state, font_med, font_small, implicit=False, trial=1, max_trials=3, time_left=None):
    """Tekent de HUD (heads-up display) onder het speelveld.

    Bevat:
    - Zettenbalk: elke cel = één zet; groen → voldoende, oranje → bijna op
    - Bananen-teller (alleen bij meerdere doelen)
    - Geest-teller (gegeten / totaal) als require_ghost_eaten actief is
    - Powerup-indicator: vakjes voor resterende powered_turns
    - Tekstuele geest-status: "Eat him now!" / "Gets you!" / "Eaten!"
    """
...
def draw_series_transition(surf, font_big, font_med, font_small, w, h, t,
    ...
    """Tussenscherm na afronden van een level-serie."""
    surf.fill(C_BG)
    cx = w // 2
    cy = int(h * 0.30)
...
def _measure_wager_preview_legend(font_big, font_small, lvl):
    """Berekent de benodigde inhoudshoogte van de wager preview legenda."""
    PAD = 14
    bh  = font_big.get_height()
    sh  = font_small.get_height()
...
def draw_wager_preview_legend(surf, font_big, font_small, lvl, x, available_y, w, available_h):
    """Legenda + ghost pad voor wager preview fase, links van het grid.
    Hoogte wordt bepaald door de inhoud; de box wordt verticaal gecentreerd."""
    PAD = 14
    bh  = font_big.get_height()
    sh  = font_small.get_height()
    ...
    # Verticaal centreren in de beschikbare ruimte
    y = available_y + max(0, (available_h - h) // 2)

    # Achtergrond met omlijning
    ...
    # Clip zodat tekst nooit buiten de box loopt
    old_clip = surf.get_clip()
...
def draw_leaderboard(surf, font_big, font_small, w, h, t, stats, name, name_confirmed,
    ...
    if lb_entries:
        ...
        # Tabelkolommen: x-positie van elke kolom
        col_rank  = cx - 300
        col_name  = cx - 260
        col_ft    = cx +  30
        col_moves = cx + 130
...
def draw_wager_leaderboard(surf, font_big, font_small, w, h, t, total_coins, lb_entries):
    """Leaderboard na de wager-game: gesorteerd op totale coins."""
    surf.fill(C_BG)
    cx = w // 2
...
def draw_wager_intro(surf, font_big, font_small, w, h, t):
    """Uitlegscherm voor de wager-game (Level 4)."""
    surf.fill(C_BG)
    cx = w // 2
...
def draw_estimate_intro(surf, font_big, font_small, w, h, t):
    """Uitlegscherm voor het estimate-type wager level (Level 4C)."""
    surf.fill(C_BG)
    cx = w // 2
...
def draw_coin_result(surf, font_big, font_small, w, h, t, info):
    """Tussenscherm na elk wagering level: coins verdiend + huidige balance."""
    surf.fill(C_BG)
    cx = w // 2
...
# ── Impliciete legenda: per level de elementen die in VORIGE levels zijn geïntroduceerd ──
# Opgebouwd uit LEVEL_LEGENDS: alleen kleur + naam, geen beschrijving.
...
# Welk level-index is het laatste van elke serie, en naar welke serie gaat het
# daarna?  Na level 5 (1F) → tussenscherm "1→2", na level 11 (2F) → "2→3", etc.
ESTIMATE_RANGES = [
    (0,   0,  "Exact!",  "Bonus +50"),
    (1,   1,  "± 1",     "Bonus +50"),
    (2,   2,  "± 2",     "Bonus +50"),
...
# Reward op basis van werkelijke gap (ongeacht gok): 50/40/30/20/10/0
...
# Eerste level-index van elke serie — gebruikt om na het tussenscherm te navigeren
SERIES_FIRST = {1: 0, 2: 6, 3: 12, 4: 18}

# Wager dirs worden gekozen op basis van implicit-flag bij aanmaken wager_session_dir


# ─── Hoofd game-loop ──────────────────────────────────────────────────────────

def main(implicit=False):
    """Start de volledige pygame-game en runt de event/render-loop.

    De loop draait op 60 fps.  Per frame:
      1. Verwerk pygame-events (toetsaanslagen, afsluiten)
      2. Verwerk spellogica als de speler een geldige zet deed
      3. Teken het huidige scherm op basis van screen_mode
      4. pygame.display.flip() — stuur het frame naar het scherm
    """
    pygame.init()

    # Laad pixel-font als het bestand bestaat; anders systeem-fallback
    ...
    # Actieve scherm-modus bepaalt wat er getekend wordt en welke input verwerkt wordt
    ...
    # ── Sessie-map (aangemaakt zodra speler start, incl. participant ID) ──────
    ...
    # ── Serietimer (30 min per serie, overschot rolt door) ────────────────────
    ...
    def _finish_game():
        ...
        if is_wager_finish:
            # Na serie 4: wager leaderboard opslaan → wager_leaderboard scherm → end_screen
    ...
    def _active_session_dir():
        """Geeft de juiste session_dir terug: wager dir voor serie 4, anders gewone dir."""
    ...
    def switch_to(idx):
        ...
        if lvl.get("estimate_level") or lvl.get("wager_level"):
            # Zorg dat wager_session_dir bestaat ook als het leaderboard overgeslagen is
    ...
    while True:
            ...
            if event.type == KEYDOWN:
                ...
                if screen_mode == "leaderboard":
                    if not name_confirmed:
                        ...
                        # Naam bevestigd → wager intro (leaderboard is altijd na serie 3)
                        if wager_session_dir is None:
                            _pid = participant_id.strip().replace(" ", "_") or "unknown"
                ...
                # ── Wager leaderboard: elke toets → end_screen ───────────────
                if screen_mode == "wager_leaderboard":
                    screen_mode = "end_screen"
                    t = 0.0
                    continue

                # ── End screen: alleen ESC sluit af ──────────────────────────
                if screen_mode == "end_screen":
                    continue

                # ── Wager intro: elke toets start level 4 ────────────────────
                if screen_mode == "wager_intro":
                    switch_to(SERIES_FIRST[4])
                    continue

                # ── Intro: tekstinvoer participant ID, ENTER om te starten ───
                ...
                # ── Tussenscherm: spatie → eerste level van volgende serie ────
                ...
                if event.key == K_r:
                    if lvl.get("wager_level") and state["status"] == "game_over":
                        # Wager level: falen = coins kwijt, door naar volgend level
                    ...
                    if not lvl.get("wager_level"):
                        if state["status"] == "game_over":
                            ...
                            # Vrijwillig resetten tijdens playing — telt ook als poging
                            fails = level_fails.get(current_level_idx, 0) + 1
                            level_fails[current_level_idx] = fails
                            moves_used = lvl["max_moves"] - state["moves_left"]
                            p = _save_run_log(lvl, level_attempts.get(current_level_idx, 1),
                ...
                if event.key == K_SPACE and state["status"] == "level_complete":
                    ...
                    if not lvl.get("estimate_level"):
                        ...
                        # 4A/4B gewonnen: coins toevoegen aan totaal
                ...
                # ── Meta-doos (schuift door tot obstakel) ─────────────────────
                ...
                # ── One-shot groene muur markeren (meta-meta green_wall_oneshot) ─
                ...
                # ── Ghost teleporters (speler alleen buiten zone) ────────────
                gtps = lvl.get("ghost_teleporters")
                if gtps:
                    pos = (state["col"], state["row"])
                ...
                # ── Ambush ghost: check activation ook tegen pre-teleport cel ─
                ...
                # ── Hinderlaag-geest botsing vóór beweging ────────────────────
        ...
        # ── Tekenen: tussenscherm tussen series ───────────────────────────────
        if screen_mode == "series_transition":
        ...
        # ── Activatiezone hinderlaag-geest ────────────────────────────────────
        ...
        # Links: expliciete legenda (alleen in expliciete modus)
        ...
        # Links: impliciete legenda (known elements + controls) in impliciete modus
        ...
        # Level titel boven het grid op het hoofdscherm (na scaling, altijd zichtbaar)
        title_surf_r = font_large.render(lvl["title"], True, C_PAC)
        screen.blit(title_surf_r, (8, 8))

        # Serietimer onder de level titel, links bovenin (niet bij wagering levels)
